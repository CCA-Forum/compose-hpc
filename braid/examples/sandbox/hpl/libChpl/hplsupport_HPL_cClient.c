...
void panelSolveNative(void* abData, void* pivData,
  ...
  if (DEBUG) printf("panelSolveNative(%p, %p, /*ab*/ %d, %d, %d, %d, /*panel*/ %d, %d, %d, %d) \n", abData, pivData,
  ...
  for (int k = start2; k <= end2; k++) {
	...
	if (DEBUG) printf(" panelSolveNative: k = %d \n", k);
	...
	if (DEBUG) printf("  panelSolveNative: pivotRow = %d, pivotVal = %f \n", pivotRow, pivotVal);
	if (pivotRow == -1) {
	  // Nothing to do
	  if (DEBUG) printf("panelSolveNative: no pivot row found, returning");
	  return;
	}

	// Swap the current row with the pivot row and update the pivot vector to reflect that
	if (pivotRow != k) {
      if (DEBUG) printf("  panelSolveNative: swapping rows in ab \n");
	  ...
	  if (DEBUG) printf("  panelSolveNative: swapping rows in piv \n");
	  // piv[k] <=> piv[pivotRow];
	  {
	...
	if (pivotVal == 0) {
      ...
      if (DEBUG) printf("panelSolveNative: Matrix cannot be factorized\n");
      return;
	}

	if (DEBUG) printf("  panelSolveNative: normalizing values of col-%d in ab \n", k);
	...
	if (DEBUG) printf("  panelSolveNative: updating remaining values of ab \n", k);
