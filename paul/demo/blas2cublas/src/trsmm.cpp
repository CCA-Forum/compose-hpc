...
void handleTRSMM(ofstream &cocciFptr,bool checkBlasCallType, bool isRowMajor, string fname, string arrayPrefix, SgExprListExp* fArgs){
	...
	if(checkBlasCallType){
		...
		cocciStream << "+  "<<cublasCall<<"("<<cblasSide<<","<<cuUplo<<","<<cuTrans<<","<<cuDiag<<",m,n,alpha,"<<arrayPrefix<<"_A,lda,"<<arrayPrefix<<"_B,ldb);  \n\n";
		...
		cocciStream << "+  cublasAlloc(lda*"<<cA<<", sizeType_"<<arrayPrefix<<", (void**)&"<<arrayPrefix<<"_A);  \n";
		...
		cocciStream << "+  cublasSetMatrix (*(m), *(n), sizeType_"<<arrayPrefix<<", (void *)"<<matBRef<<",*(m), (void *) "<<arrayPrefix<<"_B,*(m));  \n\n";
		cocciStream << "+  /* CUBLAS call */  \n";
		cocciStream << "+  "<<cublasCall<<"(*(side),*(uplo),*(transa),*(diag),*(m),*(n),*(alpha),"<<arrayPrefix<<"_A,*(lda),"<<arrayPrefix<<"_B,*(ldb));  \n\n";
		cocciStream << "+  /* Copy result array back to host */ \n";
		cocciStream << "+  cublasSetMatrix( *(m), *(n), sizeType_"<<arrayPrefix<<", (void *) "<<arrayPrefix<<"_B, *(m), (void *)"<<matBRef<<", *(m)); \n";
