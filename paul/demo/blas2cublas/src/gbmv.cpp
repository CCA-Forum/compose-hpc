...
void handleGBMV(ofstream &cocciFptr,bool checkBlasCallType, bool isRowMajor, string fname, string uPrefix, SgExprListExp* fArgs){
	...
	if(checkBlasCallType){
		...
		cocciStream << "+  "<<cublasCall<<"("<<cbTrans<<",m, n, kl, ku, alpha,"<<uPrefix<<"_A,lda,"<<uPrefix<<"_X,incx,beta,"<<uPrefix<<"_Y,incy);  \n\n";
		...
		cocciStream << "+  /* CUBLAS call */  \n";
		cocciStream << "+  "<<cublasCall<<"(*(trans),*(m), *(n), *(kl), *(ku), *(alpha),"<<uPrefix<<"_A,*(lda),"<<uPrefix<<"_X,*(incx),*(beta),"<<uPrefix<<"_Y,*(incy));  \n\n";
