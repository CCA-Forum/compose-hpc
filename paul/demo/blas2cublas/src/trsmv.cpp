...
void handleTRSMV(ofstream &cocciFptr,bool checkBlasCallType, bool isRowMajor, string fname, string uPrefix, SgExprListExp* fArgs){
	...
	if(checkBlasCallType){
		...
		cocciStream << "+  "<<cublasCall<<"("<<uplo<<","<<cbTrans<<","<<diag<<",n,"<<uPrefix<<"_A,lda,"<<uPrefix<<"_X,incx);  \n\n";
		...
		cocciStream << "+  "<<cublasCall<<"(*(uplo),*(trans),*(diag),*(n),"<<uPrefix<<"_A, *(lda),"<<uPrefix<<"_X,*(incx));  \n\n";
		cocciStream << "+  /* Copy result vector back to host */  \n";
		cocciStream << "+  cublasSetVector ( *(n), sizeType_"<<uPrefix<<","<<uPrefix<<"_X, *(incx), "<<vecXRef<<", *(incx));  \n";
	}
