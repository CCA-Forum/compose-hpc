...
bool is_task_real(double[][ichunk], double, int[], int[]);
...
void twoel(double *schwmax, double *etwo) {
    ...
    long taskid;
    long imax;
    long tottasks; // ADDED from version 1 --
    int me; // ADDED from version 1
    int nproc = 0; // ADDED from version 1

    // ADDED from version 1 -- note move to C++
    ...
    accum = 0;
    // %TASCEL version=1
    for (taskid = 0; taskid < tottasks; taskid++) {
        dotask = translate_task(taskid, lo, hi, &it, &jt, &kt, &lt);

        if (!is_task_local(g_schwarz, lo, hi, me, map, procs))
            continue;
        lo_ik[0] = lo[0];
        lo_ik[1] = lo[2];
        hi_ik[0] = hi[0];
        ...
        if (!(is_task_real(s_ij, *schwmax, lo, hi) != 0))
            continue;
...
int next_4chunk(int *lo, int *hi, int *ilo, int *jlo, int *klo, int *llo) {
    ...
    if (itask < 0) {
        printf(
                "next_4chunk: itask negative: %d imax: %ld nbfn: %ld ichunk: %d\n",
...
int translate_task(long itask, int *lo, int *hi, int *ilo, int *jlo, int *klo,
        int *llo) {
    long imax;
    int itmp;
    int ret;
    ...
    if (itask < 0) {
        printf(
                "translate_task: itask negative: %ld imax: %ld nbfn: %ld ichunk: %d\n",
                itask, imax, nbfn, ichunk);
        printf("probable GA int precision problem if imax^4 > 2^31\n");
        printf("\n");
