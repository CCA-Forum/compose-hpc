...
void handleHSBMV(ofstream &cocciFptr, bool checkBlasCallType, bool isRowMajor,
    ...
    if (checkBlasCallType)
        cocciStream << "- " << blasCall << "(order,uplo, n, k, alpha,"
                << matARef << ",lda," << vecXRef << ",incx,beta," << vecYRef
                << ",incy); \n";
    else
        cocciStream << "- " << blasCall << "(uplo, n, k, alpha," << matARef
    ...
    if (checkBlasCallType) {
        ...
        cocciStream << "+  " << cublasCall << "(" << uplo << ",n,k,  alpha,"
