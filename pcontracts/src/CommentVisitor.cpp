...
/*
 * ----------------------------------------------------------------------
 * The following routines have NOT been modified; rather slightly 
 * reformatted and moved.
 * ----------------------------------------------------------------------
 */

void 
CommentVisitor::visit(SgNode *node) {
...
string 
remove_cpp_comment_marks(const string s) {
  return s.substr(2);
}

string 
remove_c_comment_marks(const string s) {
  return s.substr(2,s.size() - 4);
}

string 
remove_f_comment_marks(const string s) {
  return s.substr(1);
}

bool 
is_annotation(const string s) {
  return s[0] == ANNOTATION_PREFIX;
}

string 
annotation_text(const string s) {
  return s.substr(1);
}

/*
 * ----------------------------------------------------------------------
 * The following routine has been "renamed" to reflect being a class
 * method AND replaces libPaul's parsers with the contracts parser.
 * ----------------------------------------------------------------------
 */
void 
CommentVisitor::handle_comment(const string s, SgLocatedNode *node, 
                               paul_tag_map tagmap) 
{
  if(is_annotation(s)) {
    ...
    if (ptm_it != tagmap.end()) {
      if ((*ptm_it).second == "contract-clause") {
	/*
	 * Contract Clause annotation
	 */
/* 
 * ToDo/TBD:  What should this look like?
 *
 * Could retain the "value" based approach by retaining the string
 * as a first pass.  
 *     But need to determine if that duplicates something already
 *     supported in a parent class
 *
 * Then there's still the matter of determining how much semantic
 * checking can (or should) be performed on contract clause contents.
 */
